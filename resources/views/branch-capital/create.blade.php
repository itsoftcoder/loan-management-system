...
@section('breadcrumb')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Create Branch Capital Page</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('branch-capital.index', $id) }}">List Branch
                                Capital</a></li>
                        <li class="breadcrumb-item active">Create Branch Capital Page</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
...
@section('content')
    <div class="card">
        <div class="card-header clearfix">
            <h3 class="card-title">Create Branch Capital</h3>
            <a href="{{ route('branch-capital.index', $id) }}" class="btn btn-xs btn-danger float-right">Branch Capital
                List</a>
        </div>
        <div class="card-body">
            <form action="{{ route('branch-capital.store', $id) }}" method="POST">
                @csrf
                <div class="row">
                    <div class="col-md-10 mx-auto">
                                ...
                                <select class="form-control" id="capital_type" name="capital_type">
                                    ...
                                    @foreach ($capital_types as $capital_type)
                                        <option value="{{ $capital_type }}"
                                            {{ old('capital_type') == $capital_type ? 'selected' : '' }}>
                                            {{ $capital_type }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <button type="submit" class="btn btn-outline-success">Save Branch Capital</button>
                            </div>
                        </div>
                    </div>
                </div>
